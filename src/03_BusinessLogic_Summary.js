...
function triggerSummaryUpdateFromEdit(e) {
  try {
    ...
    if (e.oldValue && !isNaN(new Date(e.oldValue).getTime())) {
      ...
      if (oldDate.getFullYear() > 2000) {
        // 有効な日付か簡易チェック
        editedDates.add(
          new Date(oldDate.getFullYear(), oldDate.getMonth(), oldDate.getDate()).getTime(),
        );
      }
    }
...
function _updateSummaryForDate(sheetName, date) {
  ...
  const summaryHeaderMap = createHeaderMap(
  ...
  try {
      ...
      data.forEach(row => {
        ...
        if (sheetName === TSUKUBA_CLASSROOM_NAME) {
          ...
          if (startTime instanceof Date && endTime instanceof Date) {
            ...
            const isInAfternoon =
              endHour > TSUKUBA_MORNING_SESSION_END_HOUR ||
              (endHour === TSUKUBA_MORNING_SESSION_END_HOUR && endMinutes > 0);
            if (isInAfternoon) {
    ...
    Object.keys(GOOGLE_FORM_IDS).forEach(classroomName => {
      try {
        ...
        Logger.log(`サマリー再構築後のフォーム選択肢更新に失敗 (${classroomName}): ${e.message}`);
      }
    });
...
function _createSummaryRowData(classroom, date, counts, venue, timezone) {
  ...
  if (classroom === TSUKUBA_CLASSROOM_NAME) {
    ...
    rows.push([
      `${dateString}_${classroom}_${SESSION_MORNING}`,
      date,
      classroom,
      SESSION_MORNING,
      venue,
    ...
    rows.push([
      `${dateString}_${classroom}_${SESSION_AFTERNOON}`,
      date,
      classroom,
      SESSION_AFTERNOON,
      venue,
    ...
    rows.push([
      `${dateString}_${classroom}_${ITEM_NAME_FIRST_LECTURE}`,
      date,
      classroom,
      ITEM_NAME_FIRST_LECTURE,
      venue,
      INTRO_LECTURE_CAPACITY,
      introCount,
      finalIntroAvailable,
      new Date(),
    ]);
    ...
    rows.push([
      `${dateString}_${classroom}_${ITEM_NAME_MAIN_LECTURE}`,
      date,
      classroom,
      ITEM_NAME_MAIN_LECTURE,
      venue,
      CLASSROOM_CAPACITIES[TOKYO_CLASSROOM_NAME],
      mainCount,
      availability.mainAvailable,
      new Date(),
    ]);
    rows.push([
      `${dateString}_${classroom}_${ITEM_NAME_FIRST_LECTURE}`,
    ...
    rows.push([
      `${dateString}_${classroom}_${SESSION_ALL_DAY}`,
      date,
      classroom,
      SESSION_ALL_DAY,
      venue,
