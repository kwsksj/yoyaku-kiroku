...
function updateRosterCache() {
    ...
    try {
        ...
        logActivity('user', Session.getActiveUser().getEmail(), 'CACHE_UPDATE_ROSTER', 'SUCCESS', '生徒名簿キャッシュを更新');
...
function updateRosterSheet(rosterSheet, allStudentNames) {
    ...
    rosterData.forEach(row => {
        if(row[nameColIdx] && String(row[nameColIdx]).trim()) existingNames.add(String(row[nameColIdx]).trim());
        if(row[nicknameColIdx] && String(row[nicknameColIdx]).trim()) existingNames.add(String(row[nicknameColIdx]).trim());
    });
    ...
    if (newStudents.length > 0) {
        rosterSheet.getRange(rosterSheet.getLastRow() + 1, 1, newStudents.length, newStudents[0].length).setValues(newStudents);
...
function getAllReservations() {
    ...
    const reservations = [];
    const sheetNames = [...CLASSROOM_SHEET_NAMES, ...ARCHIVE_SHEET_NAMES];
    const cacheFields = [HEADER_VENUE, HEADER_WORK_IN_PROGRESS, HEADER_IN_THE_FUTURE, HEADER_NOTES, HEADER_FROM, HEADER_LINE];
...
function updateRosterCacheColumns(rosterSheet, allReservations) {
    ...
    rosterData.forEach((rosterRow, rowIndex) => {
        ...
        const studentYears = new Set(allReservations.filter(r => r.studentId === studentId && r.date instanceof Date).map(r => r.date.getFullYear()));
...
function migrateAllRecordsToCache() {
    ...
    try {
        SpreadsheetApp.getActiveSpreadsheet().toast('全履歴のキャッシュ生成を開始しました...完了までしばらくお待ちください。', '処理中', -1);
        ...
        logActivity('user', Session.getActiveUser().getEmail(), 'CACHE_MIGRATE_RECORDS', 'SUCCESS', '全履歴から「きろく」キャッシュを生成');
...
function migrateAllFutureBookingsToCache() {
    ...
    const lock = LockService.getScriptLock();
    if (!lock.tryLock(300000)) { // 5分間ロックを試行
        ui.alert("現在、他の重い処理が実行中です。しばらく経ってから再度お試しください。");
        return;
    }

    try {
        SpreadsheetApp.getActiveSpreadsheet().toast('全生徒の「よやくキャッシュ」更新を開始しました...', '処理中', -1);
        ...
        const rosterHeaderMap = createHeaderMap(rosterSheet.getRange(1, 1, 1, rosterSheet.getLastColumn()).getValues()[0]);
        ...
        SpreadsheetApp.getActiveSpreadsheet().toast('「よやくキャッシュ」の更新が完了しました。', '完了', 10);
        ui.alert('全生徒の「よやくキャッシュ」更新が正常に完了しました。');
        logActivity('user', Session.getActiveUser().getEmail(), 'CACHE_MIGRATE_BOOKINGS', 'SUCCESS', '全予約から「よやくキャッシュ」を生成');

    } catch (e) {
        Logger.log(e);
        handleError(`「よやくキャッシュ」の一括更新中にエラーが発生しました。\n\n詳細: ${e.message}`, true);
    } finally {
        lock.releaseLock();
    }
}
