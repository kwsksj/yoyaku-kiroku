...
function setCheckboxChoices(classroomName) {
    ...
    try {
        ...
        if (choices.length > 0) {
            const item = form.getItems(FormApp.ItemType.CHECKBOX).find(i => i.getTitle() === questionTitle);
            if (item) {
...
function createStringArrayFromCounts(classroomName) {
    ...
    const headerMap = createHeaderMap(summarySheet.getRange(1, 1, 1, summarySheet.getLastColumn()).getValues()[0]);
    ...
    const choiceData = summaryData
        .filter(row => {
            ...
            return classroom === classroomName && session === ITEM_NAME_FIRST_LECTURE && date instanceof Date && date >= today;
        })
        .map(row => ({
            dateObj: row[dateIdx],
            venue: row[venueIdx] || '',
...
function addCalendarEventsToSheetWithSpecifics() {
  ...
  try {
    ...
    Logger.log(`カレンダーから予定を取得する期間: ${Utilities.formatDate(tomorrow, Session.getScriptTimeZone(), 'yyyy/MM/dd')} から ${Utilities.formatDate(oneYearLater, Session.getScriptTimeZone(), 'yyyy/MM/dd')} まで`);
    ...
    CLASSROOM_SETTINGS.forEach(setting => {
      ...
      if (!calendarId) {
        Logger.log(`エラー: シート "${sheetName}" のカレンダーIDが設定されていません。スキップします。`);
        return;
      }
      const calendar = CalendarApp.getCalendarById(calendarId);
      if (!calendar) {
        Logger.log(`エラー: カレンダー "${calendarId}" が見つかりません。この教室の処理をスキップします。`);
        return;
      }
      ...
      if (sheet.getLastRow() > 1) {
        ...
        datesInSheet.forEach(row => {
          if (row[0] instanceof Date) {
            existingDates.add(Utilities.formatDate(row[0], Session.getScriptTimeZone(), 'yyyy-MM-dd'));
          }
        });
      }
      ...
      events.forEach(event => {
        ...
        if (event.isAllDayEvent()) {
          ...
          if (event.getEndTime().getHours() === 0 && event.getEndTime().getMinutes() === 0 &&
              event.getEndTime().getSeconds() === 0 && event.getEndTime().getMilliseconds() === 0 &&
        ...
        while (currentDay.getTime() <= effectiveEndDate.getTime()) {
          const dateForComparison = Utilities.formatDate(currentDay, Session.getScriptTimeZone(), 'yyyy-MM-dd');
