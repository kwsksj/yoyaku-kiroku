...
function getSlotsAndMyBookings(studentId) {
    try {
        ...
        if (summarySheet && summarySheet.getLastRow() > 1) {
            ...
            const allSummarySlots = summaryData.map(row => {
        ...
        const sortedSlots = allSlots.sort((a, b) => new Date(a.date) - new Date(b.date) || a.classroom.localeCompare(b.classroom));
        const sortedBookings = myBookings.sort((a, b) => new Date(a.date) - new Date(b.date));

        return { success: true, availableSlots: sortedSlots, myBookings: sortedBookings };
    } catch (err) {
        Logger.log(`getSlotsAndMyBookings Error: ${err.message}\n${err.stack}`);
        return { success: false, message: `予約情報の取得中にエラーが発生しました。` };
    }
}
...
function getReservationDetails(params) {
    try {
        ...
        const details = {
            firstLecture: headerMap.has(HEADER_FIRST_LECTURE) ? rowData[headerMap.get(HEADER_FIRST_LECTURE)] === true : false,
            earlyArrival: headerMap.has(HEADER_EARLY_ARRIVAL) ? rowData[headerMap.get(HEADER_EARLY_ARRIVAL)] === true : false,
            chiselRental: headerMap.has(HEADER_CHISEL_RENTAL) ? rowData[headerMap.get(HEADER_CHISEL_RENTAL)] === true : false,
            startTime: headerMap.has(HEADER_START_TIME) && rowData[headerMap.get(HEADER_START_TIME)] instanceof Date ? Utilities.formatDate(rowData[headerMap.get(HEADER_START_TIME)], timezone, "HH:mm") : null,
            endTime: headerMap.has(HEADER_END_TIME) && rowData[headerMap.get(HEADER_END_TIME)] instanceof Date ? Utilities.formatDate(rowData[headerMap.get(HEADER_END_TIME)], timezone, "HH:mm") : null,
        };
        ...
        return { success: false, message: `予約詳細情報の取得中にエラーが発生しました。` };
    }
}
...
function getReservationDetailsForEdit(reservationId, classroom) {
    try {
        ...
        const details = {
            ...
            classroom: classroom,
            date: headerMap.has(HEADER_DATE) && row[headerMap.get(HEADER_DATE)] instanceof Date ? Utilities.formatDate(row[headerMap.get(HEADER_DATE)], timezone, 'yyyy-MM-dd') : '',
            venue: headerMap.has(HEADER_VENUE) ? row[headerMap.get(HEADER_VENUE)] : '',
            earlyArrival: headerMap.has(HEADER_EARLY_ARRIVAL) ? row[headerMap.get(HEADER_EARLY_ARRIVAL)] === true : false,
            chiselRental: headerMap.has(HEADER_CHISEL_RENTAL) ? row[headerMap.get(HEADER_CHISEL_RENTAL)] === true : false,
            startTime: headerMap.has(HEADER_START_TIME) ? timeToString(row[headerMap.get(HEADER_START_TIME)]) : '',
            endTime: headerMap.has(HEADER_END_TIME) ? timeToString(row[headerMap.get(HEADER_END_TIME)]) : '',
            workInProgress: headerMap.has(HEADER_WORK_IN_PROGRESS) ? row[headerMap.get(HEADER_WORK_IN_PROGRESS)] : '',
        ...
        return { success: false, message: `予約詳細の取得中にエラーが発生しました。` };
    }
}
...
function getAccountingMasterData() {
    try {
        const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(ACCOUNTING_MASTER_SHEET_NAME);
        ...
        return { success: false, message: '料金・商品マスタの取得中にエラーが発生しました。' };
    }
}
...
function getParticipationHistory(studentId, limit, offset) {
    try {
        ...
        return { success: false, message: '過去の参加履歴の取得中にエラーが発生しました。' };
    }
}
