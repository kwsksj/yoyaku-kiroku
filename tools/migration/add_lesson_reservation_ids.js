  ...
  try {
    ...
    for (let i = 0; i < scheduleData.length; i++) {
      ...
      if (!lessonId) {
        ...
        lessonIdUpdates.push({ row: i + 2, col: schLessonIdCol + 1, value: lessonId });
      }
    ...
    if (lessonIdUpdates.length > 0) {
      ...
      scheduleSheet.getRangeList(ranges).getRanges().forEach((range, i) => range.setValue(lessonIdUpdates[i].value));
    ...
    for (let i = 0; i < reservationData.length; i++) {
      ...
      if (expectedLessonId && row[resLessonIdCol] !== expectedLessonId) {
        reservationLessonIdUpdates.push({ row: i + 2, col: resLessonIdCol + 1, value: expectedLessonId });
      }
    ...
    if (reservationLessonIdUpdates.length > 0) {
      ...
      reservationSheet.getRangeList(ranges).getRanges().forEach((range, i) => range.setValue(reservationLessonIdUpdates[i].value));
      ...
      if (lessonId) {
        ...
        if (newIdsSortedStr !== currentIdsSortedStr) {
          scheduleReservationIdsUpdates.push({
            row: i + 2, col: schReservationIdsCol + 1, value: JSON.stringify(newReservationIds)
          });
        }
      }
    }
    if (scheduleReservationIdsUpdates.length > 0) {
      const ranges = scheduleReservationIdsUpdates.map(u => `R${u.row}C${u.col}`);
      scheduleSheet.getRangeList(ranges).getRanges().forEach((range, i) => range.setValue(scheduleReservationIdsUpdates[i].value));
