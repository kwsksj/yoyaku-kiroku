...
| `npm run test`              | Vitestユニットテストを実行                                             |
| `npm run test:changed`      | Git差分の影響を受けるテストのみ実行（高速確認用）                      |
| `npm run test:ci`           | `validate` + `test` を連続実行（CI向け）                               |
| `npm run check:plan`        | 変更差分に応じた推奨チェックコマンドを表示                             |
| `npm run check:plan:final`  | 最終確認向け（`npm run dev` 含む）推奨チェックコマンドを表示           |
| `npm run check:smart`       | 推奨チェック（必須のみ）を自動実行                                     |
| `npm run check:smart:all`   | 推奨チェック（任意含む）を自動実行                                     |
| `npm run check:smart:final` | 最終確認向け推奨チェック（必須のみ）を自動実行                         |
| `npm run smoke:gas`         | テスト環境WebAppのGAS疎通チェック（副作用なし）                        |
| `npm run smoke:gas:deep`    | `smoke:gas` + `?test=true` 画面も確認                                  |
| `npm run smoke:gas:deploy`  | テスト環境反映（`npm run dev`）後に `smoke:gas:deep` を実行            |
| `npm run pr:auto`           | PR作成/自動マージ設定/レビュー状態確認/（マージ済み時）ブランチ整理    |
