  ...
  const actionHandlers = {
    ...
    goToEditReservation: d => {
      ...
      if (reservation) {
        const editingDetails = {
          ...
          [window.HEADERS?.RESERVATIONS?.START_TIME || 'startTime']: reservation[window.HEADERS?.RESERVATIONS?.START_TIME] || reservation.startTime || '',
          [window.HEADERS?.RESERVATIONS?.END_TIME || 'endTime']: reservation[window.HEADERS?.RESERVATIONS?.END_TIME] || reservation.endTime || '',
    ...
    updateReservation: () => {
      ...
      const p = {
        ...
        [window.HEADERS?.RESERVATIONS?.START_TIME || 'startTime']: document.getElementById('res-start-time')?.value || '',
        [window.HEADERS?.RESERVATIONS?.END_TIME || 'endTime']: document.getElementById('res-end-time')?.value || '',
        workInProgress: document.getElementById('wip-input').value,
        order: document.getElementById('order-input').value,
        messageToTeacher: document.getElementById('message-input').value,
        materialInfo: document.getElementById('material-input')?.value || '',
    ...
    goToAccounting: d => {
      ...
      if (reservationData) {
        ...
        const baseDetails = {
          ...
          [window.HEADERS?.RESERVATIONS?.START_TIME || 'startTime']: reservationData[window.HEADERS?.RESERVATIONS?.START_TIME] || reservationData.startTime || null,
          [window.HEADERS?.RESERVATIONS?.END_TIME || 'endTime']: reservationData[window.HEADERS?.RESERVATIONS?.END_TIME] || reservationData.endTime || null,
        };
  ...
  actionHandlers.confirmAndPay = () => {
    ...
    if (document.getElementById('start-time')) {
      userInput.timeBased = {
        [window.HEADERS?.RESERVATIONS?.START_TIME || 'startTime']: document.getElementById('start-time').value,
        [window.HEADERS?.RESERVATIONS?.END_TIME || 'endTime']: document.getElementById('end-time').value,
  ...
  window.onload = function () {
    ...
    app.addEventListener('change', e => {
      ...
      if (e.target.name === 'experience') {
        const pastWorkContainer = document.getElementById(
          'past-work-container',
        );
