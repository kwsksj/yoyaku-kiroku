...
Google Apps Script バックエンドなしでフロントエンドを単体でテストできる開発環境を構築しています。

### クイックスタート

```bash
# 開発サーバーを起動（モックバックエンド付き）
npm run dev
...
- **Vite開発サーバー**: ホットリロード対応の高速開発環境
- **モックシステム**: `google.script.run` APIを完全エミュレート
- **ユニットテスト**: Vitestによる高速テスト実行
- **E2Eテスト**: Playwrightによるブラウザ自動テスト
- **自動環境切り替え**: 開発モード/本番モードの自動検出

### 詳細ドキュメント

詳しい使い方は [docs/FRONTEND_TESTING.md](docs/FRONTEND_TESTING.md) を参照してください。
...
**最終更新:** 2025年11月17日 (フロントエンドスタンドアロンテスト環境の追加)
