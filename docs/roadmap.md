...
このドキュメントは、GAS予約管理システムの開発・改修タスクを管理するためのロードマップです。創造主たるユーザーから提示された資料と、ソースコード分析に基づき、タスクを整理・追加し、プロジェクト全体の可視性を高めることを目的とします。
...
1. **体験価値の飛躍的向上:** 【信頼性の盤石化】フェーズが完了した今、プロジェクトの主軸は【体験価値の向上】へと移行します。FE-13（会計フローの再設計）やNF-07（代理入力機能）といった管理者体験の向上と、UI-11からUI-18に渡る一連のユーザー体験改善を並行して進めることで、システムの満足度を飛躍させます。Grokの提案として、UX/UI改善（UI-19の非同期ローディングなど）を優先的に取り入れ、GASの遅延問題を軽減。
2. **将来への布石:** NF-06（名寄せ機能）やOPS-06（過去データの一括インポート）は、システムのデータ資産価値を最大化します。また、OPS-05（自動テストの導入）は、システムの持続可能な成長に不可欠な品質保証の礎となります。ソースコードの現状（キャッシュの活用が強み）を活かし、ARC-13（CacheServiceの拡張）でパフォーマンスをさらに強化。
3. **データ整合性の継続的監視:** ARC-09（予約可否判定ロジックの統一）は、依然として重要な課題です。今後もデータフローの矛盾点を監視し、解消していくことが求められます。Grokの追加提案として、セキュリティ強化（ARC-14）を早急に実施し、電話番号認証の脆弱性を解消。全体として、GASの制約（実行時間制限）を考慮したクラウド移行（NF-10）を中長期目標に据える。
...
|:---------|:------|:-------|:----------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-------|:---------------------------------------------------|:-----------------|:---------|
| `BUG-16` | `[ ]` | 緊急   | **[会計機能] 会計処理が稀に保存できない不具合の原因調査と修正** | 会計処理時に、フロントエンドから渡される予約IDが、会計対象の予約と一致しない場合がある。この不一致の原因を特定し、正しい予約IDが参照されるように修正する。 | 高     | 05-2_Backend_Write.js, 14_WebApp_Handlers.html, 08_Utilities.js | GRP-会計機能 | -        |
...
| `OPS-05` | `[ ]` | 高     | **[品質] 自動テストの導入**                         | GAS用のテストフレームワークを導入し、主要なバックエンド関数の単体テストを作成する。                                                                                        | 高     | (テスト用ファイル)                                        | GRP-運用基盤     | -        |
...
| `ARC-14` | `[ ]` | 高     | **[セキュリティ] 認証強化と秘密管理**               | 電話番号認証にOTP追加。ハードコードIDをPropertiesServiceに移行。                                                                                                           | 高     | 04_Backend_User.gs, 08_Utilities.gs                | GRP-データ基盤      | -        |
...
| `ARC-17` | `[ ]` | 高     | **[アーキテクチャ] 会計後続処理の非同期化**         | 【BUG-16の調査結果に依存】ロック競合が主要因と特定された場合に実施する、最も効果的な解決策。ユーザーへの応答に必須でない処理を非同期化し、ロック保持時間を最小限に抑える。 | 高     | 05-2_Backend_Write.js, (新規トリガー関数)              | GRP-会計機能     | BUG-16   |
...
| `UI-15`  | `[ ]` | 中     | **[UX改善] 制作メモのインライン編集**               | 制作メモの表示エリアをタップすることで、直接編集モードに切り替わるようにする。                                                                                             | 中     | 13_WebApp_Views.html, 14_WebApp_Handlers.html      | GRP-参加記録機能 | -        |
| `NF-08`  | `[ ]` | 中     | **[予約機能] 先生へのメッセージ機能**               | 予約申し込み時に、先生へのメッセージを任意で入力できる欄を追加する。                                                                                                       | 中     | 05-2_Backend_Write.gs, 13_WebApp_Views.html        | GRP-予約体験改善 | -        |
...
| `NF-01`  | `[ ]` | 中     | **[会計機能] 電話番号なしでの会計対応**             | 体験参加者など、電話番号未登録でも会計処理を可能にする。                                                                                                                   | 中     | 04_Backend_User.gs, 13_WebApp_Views.html           | GRP-会計機能     | -        |
...
| `ARC-01` | `[ ]` | 低     | **[データモデル] 予約IDの役割の再定義**             | 予約IDを「人」ではなく「席」に紐づく固定IDとして再設計する。                                                                                                               | 最高   | (全ファイル)                                           | GRP-データ基盤      | -        |
