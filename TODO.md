...
- 同じ電話番号で２重にアカウントが作られてしまう場合がある
- 予約レンダーで、当日の日程は、終了時間以後は表示しない、予約できないようにする
- 受講済みの予約日程に予約できてしまうバグ（予約日変更で？）
- 終了フローで、表示される予約日程は、必ず経験者の日程を表示するようにする（初回の終了フロー中は、まだ会計が終わっていないので、初回者の日程が表示されてしまっている。）
- 参加者ビューで、よやくには、「完了」の人は表示しない
- 完了済みのよやくが、何故かキャンセルされてしまったので、それができないようにする
- セッション終了フローからの予約だと、予約シートに会場名が記載されない？
- キャンセルがうまくいかない報告
- ログアクションの色分け
