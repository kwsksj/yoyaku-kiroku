...
- メッセージに対して、即メールとして返信を書けるような仕組みメールを送り逃した人への対応
- 終了フローかんそうに対して、私のフィードバックが送れるような仕組み
  - 書いたら、メール通知？
...
- [x] **予約画面の初期選択フロー調整** (2026-01-28)
  - 教室選択後に日程を表示
  - 参加枠トグルは教室選択後に表示（初回のみ）
  - 参加枠トグルの初期値は「初回」
- [x] **夜教室対応（開室時間表示・初回文言・夜バッジ）** (2026-01-28)
  - 予約一覧・終了フローの予約一覧に開室時間を表示
  - 初回枠の開始時刻と案内文言を表示
  - 夜教室はバッジ表示、ダッシュボードの予約リストにも反映
- [x] **よやく詳細画面の販売品項目の見え方調整** (2026-01-22)
  - 会計画面スタイルに統一（Components.sectionHeader使用）
  - チェックボックス行を左右配置レイアウトに変更
- [x] **新規登録画面にウェルカムメッセージ表示** (2026-01-21)
  - 新規登録時に導入モーダルを表示し、ユーザーを歓迎
  - UX改善、雰囲気作り
- [x] **初回講習完了時の未来予約フラグ自動更新と空き通知** (2026-01-20)
  - 完了時に未来の初回予約を経験者へ変更
  - 変更により空いた初回枠の待機者へ通知
  - 経験者へ変更されたキャンセル待ちユーザーが枠に入れる場合の通知

- [x] **電話番号重複チェックの強化（シート直接参照）** (2026-01-19)
- [x] **不要なconsole.log削除 (debugLogへの置換)** (2026-01-21)
  - 開発ログを `debugLog` に統一し、`?debug=true` で制御可能に改善
  - サーバーサイドからのパラメータ渡しに対応
