...
- リファクタリング
  - 不要なコードを削除する
- 予約画面で出てくる「参加枠の表示」切り替えボタンの表示を、過去に予約がない人に限定する
- 予約画面の表示を、セッション終了フローの予約画面と似たデザインにする
- PC表示に場合の色々な表示を調整する。参加者ビューは、PC向けの表示も維持する
- セッション終了フローの予約画面で予約済みのばあい、進むボタンが表示されないバグ
...
- [x] 売上の別シートへの自動転載が機能していない
  - ログシートには転載した旨が書いてあるのに、実際は転載されていない
  - シートのメニューボタンからの「日付を指定して転載」は機能する
- [x] ログシートに、何も書かれていない空行が生じている
  - 本来、ログは最上部に行を挿入していたが、同時に書き込むと行の挿入と書き込みの間に他の処理が割り込み、空行ができる
  - 対策として、一番下の行に追加する方式に変更した
