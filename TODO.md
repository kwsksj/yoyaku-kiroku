...
- 画面のリロード時に、表示されているページ自体は維持されるが、情報がなくなってしまってログがないと表示されてしまう。正しく表示するには、ログアウトとログインを再度しなければならなくなっている。これを修正する
- 初回申込みか否かの判定が怪しくなっています。参加記録がある人が、初回として申し込まれていました。
- 初回申込みの際に、予約画面で表示される、「初回か、２回目以降か」の切り替えスイッチは、参加記録がない場合のみ表示されるべきです。（この機能は、このシステムを遣い出す前からの生徒さんが、参加する場合に対応できるようにするためのものです）
- この切り替えスイッチのデザインを、セッション終了フローの予約画面の日程一覧の教室切り替えボタンのデザインにする（コンポーネント化）
