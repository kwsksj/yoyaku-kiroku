 ...
 *
 * バックエンドでの会計計算結果
 * AccountingDetailsCoreのエイリアス
 *
 * @example
 * ```typescript
 * const resultDto: AccountingCalculationResultDto = {
 *   tuition: {
 ...
 * ```
 */
type AccountingCalculationResultDto = AccountingDetailsCore;

/**
 * 会計マスターアイテム応答DTO
 *
 * 会計マスター取得時のAPI応答
 * AccountingMasterItemCoreのエイリアス
 */
type AccountingMasterItemDto = AccountingMasterItemCore;

/**
 * 会計マスター分類応答DTO
 *
 * 分類済み会計マスターのAPI応答
 * ClassifiedAccountingItemsCoreのエイリアス
 */
type ClassifiedAccountingItemsDto = ClassifiedAccountingItemsCore;
